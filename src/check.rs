...
pub fn execute(onlycheck: bool, onlyrustcheck: bool, onlydircheck: bool, rmcheck: bool) {
    ...
    if rmcheck {
        ...
        if dirs.len() != 0 {
            println!("[CHECK] If you want to delete these directory, please execute --rmcheck option");
        }
...
fn search_iddir() -> Vec<String> {
    ...
    if dirs.len() != 0 {
        ...
        for i in 0..dirs.len() {
            ...
            println!("[CHECK] {} is seems to be directory created by {}", dirs[i], env!("CARGO_PKG_NAME"));
        ...
        println!("[CHECK] The diectory created by {} seemed to be absent", env!("CARGO_PKG_NAME"));
...
fn rm_iddir(dirs: Vec<String>) {
    ...
    println!("[REMOVED] All directories that {} may have created were successfully deleted", env!("CARGO_PKG_NAME"));
